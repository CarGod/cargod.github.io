import { videoHomeCard, renderVideo, videoPath, videoAlternates, videoCopy } from './lib/video.mjs';
...
import { councilMethods, renderMethodCard } from "./lib/council-cards.mjs";
import { renderCouncil, councilHomeCard } from "./lib/council.mjs";
...
async function build() {
  ...
  for (const locale of ['zh-CN','en']) {
    const url=videoPath(locale),t=videoCopy[locale];
    await write(`${url.slice(1)}index.html`,renderVideo({locale,head:baseHead({title:t.title,description:t.description,canonical:`${SITE_URL}${url}`,locale,alternates:videoAlternates}),header:siteHeader(locale,'',videoAlternates),footer:siteFooter(locale)}));
